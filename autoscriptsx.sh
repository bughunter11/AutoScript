...
EOF
    ...
    sed -i "s/IP_PLACEHOLDER/$(curl -s ifconfig.me || echo "N/A")/g" /home/$username/service_info.txt
    sed -i "s/DOMAIN_PLACEHOLDER/$(cat $DOMAIN_FILE 2>/dev/null || echo "not-set")/g" /home/$username/service_info.txt
}
...
main_menu() {
    ...
    while true; do
        ...
        echo -e "${GREEN}1.${N} Create Account"
        echo -e "${GREEN}2.${N} Delete Account"
        echo -e "${GREEN}3.${N} Renew Account"
        echo -e "${GREEN}4.${N} Lock/Unlock Account"
        echo -e "${GREEN}5.${N} Edit Banner"
        echo -e "${GREEN}6.${N} Edit 101 Response"
        echo -e "${GREEN}7.${N} Change Domain"
        echo -e "${GREEN}8.${N} Manage Services (Including SlowDNS)"
        echo -e "${GREEN}9.${N} System Info"
        echo -e "${CYAN}10.${N} Install Required Services"
        echo -e "${CYAN}11.${N} Backup System"
        echo -e "${CYAN}12.${N} Restore Backup"
        echo -e "${CYAN}13.${N} Update Script"
        echo -e "${RED}0.${N} Exit"
        echo ""
